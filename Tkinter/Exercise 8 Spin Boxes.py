...
'''create spin box
"from_" is a special keyword for Python we need to add _ after it'''
year = StringVar()
#Spinbox(root,from_=1990, to=2022, textvariable=year).grid(row=6,column=1)
'''use grid geometry manager for our window
problem: we are able to edit the values in the spinbox so we need to add the "readonly" function'''

Spinbox(root, from_=1990, to=2022, textvariable=year, state="readonly").grid(row=6,column=1)

'''to print the value when the program is run, add a print statement in the callback function
print(year.get())'''
